...
class UserSettings extends React.Component {
    ...
    handleUpdateGiveaway = () => {
      let update_entry = this.state.new_entry_options
      update_entry = update_entry.map((give, index) => {
        if (index === this.state.edit_giveaway_index) {
          return {
            ...give,
            entry_url: this.state.update_giveaway
          }
        } else {
          return give
        }
      ...
      this.setState({
        update_giveaway: '',
        edit_giveaway: '',
        new_entry_options: update_entry
      })
  	}
    handlePreviewGiveaway = () => {
      var url = document.getElementById('giveawayurl');
